...
Move `scripts/claudechain/prompts/summary_prompt.md` to `src/claudechain/resources/prompts/summary_prompt.md` to align with conventional Python package structure and eliminate the need for a symlink.

## Motivation

### Current State Issues

1. **Unconventional Location**: Prompt template is in `scripts/claudechain/prompts/` instead of within the main package at `src/claudechain/`
2. **Symlink Dependency**: A symlink exists at `src/claudechain/prompts -> ../../scripts/claudechain/prompts` for "backward compatibility" (per architecture-update.md:514)
3. **Inconsistent with Package Structure**: Main codebase is in `src/claudechain/` but resource files are outside it
...
1. **Standard Python Package Layout**: Resources live alongside code in `src/claudechain/resources/`
...
│       └── summary_prompt.md  # MOVED from scripts/claudechain/prompts/
...
└── claudechain/         # Can be removed entirely if empty
...
**Tasks**:
- [x] Create `src/claudechain/resources/` directory
- [x] Create `src/claudechain/resources/prompts/` directory
- [x] Create `src/claudechain/resources/__init__.py` (empty file for package recognition)
- [x] Create `src/claudechain/resources/prompts/__init__.py` (empty file for package recognition)
- [x] Copy `scripts/claudechain/prompts/summary_prompt.md` to `src/claudechain/resources/prompts/summary_prompt.md`

**Files Created**:
- `src/claudechain/resources/__init__.py`
- `src/claudechain/resources/prompts/__init__.py`
- `src/claudechain/resources/prompts/summary_prompt.md`
...
**Tasks**:
- [x] Update `src/claudechain/cli/commands/prepare_summary.py`:
  - Change path construction from `os.path.join(action_path, "scripts/claudechain/prompts/summary_prompt.md")`
  - To use new resources path `src/claudechain/resources/prompts/summary_prompt.md`
...
**Technical Notes**:
- Updated template path from `scripts/claudechain/prompts/summary_prompt.md` to `src/claudechain/resources/prompts/summary_prompt.md`
...
- [x] Update `tests/unit/cli/commands/test_prepare_summary.py`:
  - Change all references from `tmp_path / "scripts" / "claudechain" / "prompts" / "summary_prompt.md"`
  - To `tmp_path / "src" / "claudechain" / "resources" / "prompts" / "summary_prompt.md"`
...
- [x] Update `docs/architecture/architecture.md`:
  ...
  - To `src/claudechain/resources/prompts/summary_prompt.md`
...
**Tasks**:
- [x] Remove symlink: `src/claudechain/prompts`
- [x] Remove file: `scripts/claudechain/prompts/summary_prompt.md`
- [x] Remove directory: `scripts/claudechain/prompts/` (if empty)
- [x] Remove directory: `scripts/claudechain/` (if empty)
- [x] Remove directory: `scripts/` (if empty)

**Files Deleted**:
- `src/claudechain/prompts` (symlink)
- `scripts/claudechain/prompts/summary_prompt.md`
- `scripts/claudechain/prompts/` (directory)
- `scripts/claudechain/` (directory)
- `scripts/` (directory)

**Technical Notes**:
- All old structure successfully removed
- No symlinks remain in `src/claudechain/`
...
- [x] Verify CLI command works: `python3 -m claudechain prepare-summary` (in test environment)
...
- All 9 tests in `test_prepare_summary.py` pass successfully
- Full test suite passes with 84.72% coverage (above 70% requirement)
- Template file verified at `src/claudechain/resources/prompts/summary_prompt.md`
- Old `scripts/` directory confirmed removed
- Old symlink `src/claudechain/prompts` confirmed removed
...
|------|--------|-------------|
| `src/claudechain/resources/__init__.py` | Create | Package marker for resources |
| `src/claudechain/resources/prompts/__init__.py` | Create | Package marker for prompts |
| `src/claudechain/resources/prompts/summary_prompt.md` | Create | Moved template file |
| `src/claudechain/cli/commands/prepare_summary.py` | Modify | Update path resolution logic |
...
| `src/claudechain/prompts` | Delete | Remove symlink |
| `scripts/claudechain/prompts/summary_prompt.md` | Delete | Remove old file |
| `scripts/claudechain/prompts/` | Delete | Remove directory |
| `scripts/claudechain/` | Delete | Remove directory (if empty) |
...
- Update path construction to use `src/claudechain/resources/prompts/summary_prompt.md`
...
### Integration Tests
- Run ClaudeChain locally with `prepare-summary` command
...
- Keep symlink but move `scripts/claudechain/prompts/` to `src/claudechain/prompts/`
...
- Current symlink: `src/claudechain/prompts -> ../../scripts/claudechain/prompts`
- Template usage: `src/claudechain/cli/commands/prepare_summary.py:47`
