...
Currently, ClaudeChain has an inconsistency in how spec files are accessed. The statistics workflow is failing because it's trying to read spec files from the filesystem (e.g., `claude-chain/test-project-0d044eb4/configuration.yml`), but these files don't exist in the main branch - they were only created during E2E test runs and never merged.
...
1. Users must merge their spec before running ClaudeChain workflows
...
**User Requirements:**
- When ClaudeChain workflow runs, return clear error if spec doesn't exist in main branch
...
- ✅ Update `.github/workflows/claudechain.yml` to accept and pass `base_branch` input
- ✅ Update `.github/workflows/claudechain-statistics.yml` to accept and pass `base_branch` input
...
- `.github/workflows/claudechain.yml` - Already had base_branch input configured
- `.github/workflows/claudechain-statistics.yml` - Added base_branch input to workflow_dispatch (lines 14-17) and passed to statistics action (line 36)
...
- The `.github/workflows/claudechain.yml` already had base_branch support configured for E2E testing
...
**Tasks:**
- ✅ Add function to `src/claudechain/infrastructure/github/operations.py`:
...
**Files Modified:**
- `src/claudechain/infrastructure/github/operations.py` - Added two new functions (lines 130-179)
...
**Tasks:**
- ✅ Update `src/claudechain/cli/commands/prepare.py`:
  - ✅ Get base branch from environment (default: "main")
  - ✅ Before loading spec, check if `claude-chain/{project}/spec.md` exists in base branch
  - ✅ Check if `claude-chain/{project}/configuration.yml` exists in base branch
  - ✅ If either doesn't exist, return error with clear message:
    ```
    Error: Spec files not found in branch '{base_branch}'
    Required files:
      - claude-chain/{project}/spec.md
      - claude-chain/{project}/configuration.yml

    Please merge your spec files to the '{base_branch}' branch before running ClaudeChain.
    ```
  - ✅ Exit with code 1 (error)

**Files Modified:**
- `src/claudechain/cli/commands/prepare.py` - Added validation logic at lines 55-81
- `tests/integration/cli/commands/test_prepare.py` - Updated all tests to mock `file_exists_in_branch` and added new test case

**Technical Notes:**
- Added import for `file_exists_in_branch` from `claudechain.infrastructure.github.operations` (line 11)
...
**Expected Outcome:**
✅ Users get clear error message if they try to run ClaudeChain without spec in base branch
...
**Tasks:**
- ✅ Update `src/claudechain/cli/commands/prepare.py`:
  - ✅ Replace `open(spec_path, "r")` with `get_file_from_branch(repo, base_branch, f"claude-chain/{project}/spec.md")`
  - ✅ Use API to read spec content instead of filesystem
- ✅ Update `src/claudechain/application/collectors/statistics_collector.py`:
...
- ✅ Update `src/claudechain/domain/config.py`:
...
- ✅ Update `src/claudechain/application/services/task_management.py`:
...
- ✅ Update `src/claudechain/cli/commands/finalize.py`:
  - ✅ Removed spec marking call
  - ✅ Added comment explaining specs are now in main branch

**Files Modified:**
- `src/claudechain/cli/commands/prepare.py` - Now fetches spec and config via GitHub API
- `src/claudechain/application/collectors/statistics_collector.py` - All functions use GitHub API
- `src/claudechain/domain/config.py` - Added string-based variants of load functions
- `src/claudechain/application/services/task_management.py` - Supports both file paths and string content
- `src/claudechain/cli/commands/finalize.py` - Removed spec file marking
...
**Files Modified:**
- `src/claudechain/application/collectors/statistics_collector.py` - Added missing `run_gh_command` import
...
**Tasks:**
- ✅ Delete all files in `claudechain-metadata` branch under `projects/` directory:
...
- ✅ Done via git commands on claudechain-metadata branch
- ✅ Document that this is a one-time cleanup for the refactor

**Files Modified:**
- Deleted 5 project JSON files from `claudechain-metadata` branch (committed as f6ddbca)

**Technical Notes:**
- Switched to the `claudechain-metadata` branch locally using `git checkout`
...
- Pushed changes directly to the `claudechain-metadata` branch
...
- ✅ Update `README.md`:
  ...
  - ✅ Explain workflow: Create spec → Merge to main → Run ClaudeChain
...
**Technical Notes:**
- README.md now clearly explains in Step 5 that spec files must exist in base branch before running ClaudeChain
- Added workflow diagram: "Create spec files → Merge to base branch → Run ClaudeChain"
...
- ✅ Create a permanent test project in main branch:
  - Location: `claude-chain/e2e-test-project/`
...
**Files Created:**
- `claude-chain/e2e-test-project/spec.md` - 310 tasks
- `claude-chain/e2e-test-project/configuration.yml` - Test config (maxOpenPRs: 5)
- `claude-chain/e2e-test-project/pr-template.md` - Template
...
**Technical Notes:**
- Created permanent test project at `claude-chain/e2e-test-project/` with 310 simple tasks
...
2. Ensure spec files are in `claude-chain/{project}/` directory structure
3. Run ClaudeChain workflows as normal (they will now fetch specs from main)
