...
Changes needed:
- Modify the summary prompt template at `src/claudechain/resources/prompts/summary_prompt.md`
...
**Technical notes:**
- Modified `src/claudechain/resources/prompts/summary_prompt.md` to instruct Claude Code to write summary to `/tmp/pr-summary.md` instead of posting via `gh pr comment`
...
Refactor `src/claudechain/cli/commands/prepare_summary.py`:
...
Files to modify:
- `src/claudechain/cli/commands/prepare_summary.py` - add cost extraction logic
...
**Technical notes:**
- Modified `src/claudechain/cli/commands/prepare_summary.py` to:
...
Create new command `src/claudechain/cli/commands/post_pr_comment.py`:
...
**Technical notes:**
- Created new command `src/claudechain/cli/commands/post_pr_comment.py` with:
...
- Registered command in CLI:
  - Added import in `src/claudechain/__main__.py`
  - Added routing in main() function
  - Added parser definition in `src/claudechain/cli/parser.py`
...
- Build succeeds: `python3 -m claudechain --help` shows new `post-pr-comment` command
...
- Add new step after summary generation: "Post PR summary and cost"
  - Runs Python command: `python3 -m claudechain post-pr-comment`
...
- Added new step "Post PR summary and cost" (lines 183-199):
  - Uses `python3 -m claudechain post-pr-comment` command
...
- Build succeeds: `python3 -m claudechain --help` shows all commands
...
- Mark `src/claudechain/cli/commands/add_cost_comment.py` as deprecated or remove entirely (replaced by post_pr_comment.py)
- Mark `src/claudechain/cli/commands/extract_cost.py` as deprecated if no longer needed independently
...
- Removed obsolete command files:
  - Deleted `src/claudechain/cli/commands/add_cost_comment.py` (fully replaced by post_pr_comment.py)
...
- Refactored `src/claudechain/cli/commands/extract_cost.py`:
...
- Updated CLI routing in `src/claudechain/__main__.py`:
...
- Updated CLI parser in `src/claudechain/cli/parser.py`:
...
- Build succeeds: `python3 -m claudechain --help` shows only active commands
...
- Verify ClaudeChain footer is present
...
- **Build verification:** `python3 -m claudechain --help` succeeds and shows all commands including new `post-pr-comment`
