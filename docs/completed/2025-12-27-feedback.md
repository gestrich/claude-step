...
**Technical Notes:**
- Updated Overview to explain "ClaudeChain" name and PR chain mechanism (README.md:3-7)
...
### Overview
- Clarify why it's called "ClaudeChain": It runs Claude Code on individual steps that you define for your project, creating pull requests for each step one at a time
...
**Technical Notes:**
- Renamed workflow file reference from `ai-refactor.yml` to `claude-chain.yml` (README.md:66)
...
### Workflow File
- Rename from `ai-refactor.yml` to `claude-chain.yml`
...
- Removed note "All PRs are automatically labeled with `claudechain` for tracking purposes" (README.md:353)
- `pr_label` remains in inputs section with default value 'claude-chain' (already correct in README.md:237)
...
- Remove note "All PRs are automatically labeled with `claudechain` for tracking purposes" (not true since label can be overridden)
