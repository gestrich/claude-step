...
ClaudeChain previously had AI-generated PR summary functionality that would analyze the diff and post a summary comment to each PR. This feature was added in commit e680d1f and later enhanced with cost tracking in commit ab68ce6.
...
- The summary prompt template exists at `src/claudechain/resources/prompts/summary_prompt.md`
...
- .github/workflows/claudechain.yml - ClaudeChain invocation (does not explicitly set add_pr_summary, uses default)
- src/claudechain/cli/commands/prepare_summary.py - Prompt generation (working correctly)
- src/claudechain/infrastructure/github/actions.py - Output handling (working correctly)
...
1. **Enhanced Summary Prompt** (`src/claudechain/resources/prompts/summary_prompt.md`)
...
✅ **Footer:** All include "Generated by ClaudeChain • [View workflow run]" with URL
...
The summary format and content quality are **excellent** when summaries are posted. The template in `src/claudechain/resources/prompts/summary_prompt.md` is producing high-quality, well-formatted summaries that meet all requirements:
...
Files examined:
- `src/claudechain/resources/prompts/summary_prompt.md` - Template is well-structured and producing quality output
...
1. **Updated Summary Validation** (`tests/e2e/test_workflow_e2e.py:111-125`)
   ...
   - Added validation for ClaudeChain footer containing "Generated by ClaudeChain" and "View workflow run"
...
```python
# The AI summary uses the format "## AI-Generated Summary" with a ClaudeChain footer
...
# Verify the summary has the ClaudeChain footer
has_claudechain_footer = any(
    "Generated by ClaudeChain" in body and "View workflow run" in body
...
2. The ClaudeChain footer with workflow run link (ensures proper attribution and traceability)
...
- Test PR: #123 (https://github.com/gestrich/claude-chain/pull/123)
...
Found 1 comment(s). PR URL: https://github.com/gestrich/claude-chain/pull/123
```
