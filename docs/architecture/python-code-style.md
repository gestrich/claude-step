...
### Example: StatisticsService Refactoring

The `StatisticsService` refactoring (documented in `docs/proposed/refactor-statistics-service-architecture.md`) demonstrates the "parse once into well-formed models" principle in action.

**Before: Parsing JSON in Service Layer** ❌
```python
class StatisticsService:
    def collect_team_member_stats(self, days_back: int, label: str):
        # BAD: Service layer makes GitHub API calls
        merged_prs_json = run_gh_command(
            f"pr list --state merged --json number,title,assignees,mergedAt"
        )
        open_prs_json = run_gh_command(
            f"pr list --state open --json number,title,assignees,createdAt"
        )

        # BAD: Service layer parses JSON
        merged_prs = json.loads(merged_prs_json)
        open_prs = json.loads(open_prs_json)

        # BAD: Service layer navigates dictionaries with string keys
        stats = {}
        for pr in merged_prs:
            # No type safety - string keys, runtime errors possible
            reviewer = pr["assignees"][0]["login"]
            pr_number = pr["number"]
            title = pr["title"]

            # Dictionary construction in service
            stats[reviewer]["merged"].append({
                "number": pr_number,
                "title": title,
...
**Problems:**
- Service layer knows about GitHub CLI command syntax
- JSON parsing scattered throughout service methods
- No type safety (string keys, missing field errors at runtime)
- Hard to test (must mock `run_gh_command` and JSON responses)
- Violates single responsibility (business logic + parsing)
...
@dataclass
class PRReference:
    """Domain model for PR information in statistics"""
    pr_number: int
    title: str
    project: str
    timestamp: datetime
...
class TeamMemberStats:
    ...
    username: str
    merged_prs: List[PRReference]  # Type-safe!
    open_prs: List[PRReference]    # Type-safe!

    def add_merged_pr(self, pr_ref: PRReference):
        """Type-safe API"""
        self.merged_prs.append(pr_ref)

# 2. Service Layer (services/statistics_service.py)
class StatisticsService:
    """Service uses domain models - no parsing logic"""
    ...
    def collect_team_member_stats(
        ...
        """Clean service logic - no JSON parsing"""
        stats_by_reviewer: Dict[str, TeamMemberStats] = {}

        # GOOD: Query metadata service (single source of truth)
        projects = self.metadata_service.list_project_names()

        for project in projects:
            metadata = self.metadata_service.get_project(project)

            # GOOD: Type-safe access to pull requests
            for pr in metadata.pull_requests:
                reviewer = pr.reviewer  # Type-safe property access
                task_desc = self._get_task_description(project, pr.task_index)

                # GOOD: Parse once into domain model
                pr_ref = PRReference.from_metadata_pr(pr, project, task_desc)

                # GOOD: Type-safe stats update
                if reviewer not in stats_by_reviewer:
                    stats_by_reviewer[reviewer] = TeamMemberStats(reviewer)
...
**Benefits achieved:**
- ✅ **Parse once**: Metadata → `PRReference` domain model at boundary
- ✅ **Type safety**: `pr.reviewer`, `pr.merged_at` are typed properties
- ✅ **No JSON parsing**: Service works with domain models, not dictionaries
- ✅ **Infrastructure layer separation**: GitHub API calls in infrastructure
- ✅ **Single source of truth**: Metadata service provides all data
- ✅ **Testable**: Mock domain models, not JSON strings
- ✅ **Centralized validation**: Domain model constructors validate structure
- ✅ **Reusable**: `PRReference` can be used across services

**Testing becomes clean:**
```python
def test_collect_team_member_stats():
    # Arrange: Create domain models directly
    mock_metadata = HybridProjectMetadata(
    ...
    mock_metadata_service.get_project.return_value = mock_metadata

    # Act: Service works with typed models
    stats = service.collect_team_member_stats(days_back=30)

    # Assert: Type-safe access
    assert "alice" in stats
    assert len(stats["alice"].merged_prs) == 1
    assert stats["alice"].merged_prs[0].pr_number == 123
```

For the complete refactoring journey (10 phases), see `docs/proposed/refactor-statistics-service-architecture.md`.
