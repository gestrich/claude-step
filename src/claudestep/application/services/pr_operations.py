...
def format_branch_name(project_name: str, index: int) -> str:
    ...
    Examples:
        >>> format_branch_name("my-refactor", 1)
        'claude-step-my-refactor-1'
        >>> format_branch_name("swift-migration", 5)
...
def parse_branch_name(branch: str) -> Optional[Tuple[str, int]]:
    ...
    Examples:
        >>> parse_branch_name("claude-step-my-refactor-1")
        ('my-refactor', 1)
        >>> parse_branch_name("claude-step-swift-migration-5")
        ('swift-migration', 5)
        >>> parse_branch_name("invalid-branch")
        None
    """
...
def get_project_prs(
    project_name: str, repo: str, state: str = "all", label: str = "claudestep"
) -> List[dict]:
    ...
    Examples:
        ...
        >>> prs = get_project_prs("my-refactor", "owner/repo", state="open")
        >>> len(prs)
        3
        >>> prs[0]["headRefName"]
        'claude-step-my-refactor-1'
