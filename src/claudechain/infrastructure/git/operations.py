...
def detect_changed_files(ref_before: str, ref_after: str, pattern: str) -> List[str]:
    ...
    Args:
        ...
        pattern: File pattern to filter (e.g., "claude-chain/*/spec.md")
...
def parse_spec_path_to_project(path: str) -> Optional[str]:
    ...
    Examples:
        >>> parse_spec_path_to_project("claude-chain/my-project/spec.md")
        'my-project'
        >>> parse_spec_path_to_project("claude-chain/another/spec.md")
        'another'
    ...
    if parts[0] != "claude-chain" or parts[2] != "spec.md":
        return None
