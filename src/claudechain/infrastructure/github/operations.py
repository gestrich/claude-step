...
from claudechain.infrastructure.git.operations import run_command
...
def list_pull_requests(
    ...
    **Pagination Note**:
    ...
    Args:
        ...
        label: Optional label filter (e.g., "claudechain" for ClaudeChain PRs)
    ...
    Example:
        >>> # Check capacity
        >>> prs = list_pull_requests("owner/repo", state="open", label="claudechain", assignee="alice")
        >>> print(f"Assignee has {len(prs)} open PRs")
        >>>
        >>> # Statistics for large repos
        >>> all_prs = list_pull_requests("owner/repo", state="all", label="claudechain", limit=500)
...
def list_merged_pull_requests(
    ...
    **Future Usage**: Useful for:
    ...
    Args:
        ...
        label: Optional label filter (e.g., "claudechain")
    ...
    Example:
        ...
        >>> recent_merged = list_merged_pull_requests("owner/repo", since=cutoff, label="claudechain")
...
def list_open_pull_requests(
    ...
    **Usage Examples**:
    ...
    Example:
        >>> # Check capacity
        >>> open_prs = list_open_pull_requests("owner/repo", label="claudechain", assignee="alice")
...
def list_pull_requests_for_project(
    ...
    **Usage Examples**:
    ...
    branch_prefix = f"claude-chain-{project_name}-"
...
def list_branches(repo: str, prefix: Optional[str] = None) -> List[str]:
    ...
    Args:
        repo: GitHub repository (owner/name)
        prefix: Optional prefix to filter branches (e.g., "claude-chain-")
